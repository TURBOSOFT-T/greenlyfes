               ...
               <div class="col-xxl-12">
                  <div class="breadcrumb__content z-index text-center">
                     <div class="breadcrumb__section-title-box">
                        <h3 class="breadcrumb__title">Les hopitaux</h3>
                     </div>
               ...
               <div class="row">
                  <div class="col-xl-4 col-lg-4">
                     <div class="tp-product-sidebar">
                        ...
                        <div class="tp-product-widget mb-30">
                           ...
                           <div class="sidebar__widget-content">
                              <div class="sidebar__search">
                                 <form role="search" action="{{ url('recherche') }}" method="get">
                                    @csrf
                                    <div class="sidebar__search-input-2">
                                       <input type="text" id="search"  type="search" name="search"    value="{{ $title ?? '' }}"
                                       placeholder="Recherche hopital">
                        ...
                        <!-- categories -->
                        <div class="tp-product-widget mb-30">
                           <h3 class="sidebar__widget-title mb-0">Les spécialités </h3>
                           <div class="tp-product-widget-content">
                              <div class="tp-product-widget-category">
                                 <ul>
                                    @foreach ($specialites as $category)
                                    <li><a  href="/specialite/{{ $category->id }}"
                                        class="{{ isset($current_specialite) && $current_specialite->id === $category->id ? 'selected' : '' }}">  {{ Str::limit($category->title,25) }}<span>{{ $category->hopitals->count() }}</span></a></li>
                              @endforeach
                                 </ul>
                              </div>
                           </div>
                  ...
                  <div class="col-xl-8 col-lg-8">
                     <div class="row">  
                        @foreach($hopitaux as $product)
                        @if ($hopitaux)
                        <div class="col-xl-4 col-md-6 col-sm-6">
                           <div class="tp-product-item-2 mb-40">
                              <div class="tp-product-thumb-2 p-relative z-index-1 fix w-img">
                                 <a href="{{ route('details-produits', ['id' => $product->id, 'slug' => Str::slug(Str::limit($product->title, 10))]) }}">
                              ...
                              <div class="tp-product-content-2">
                                 ...
                                 <h3 class="tp-product-title-2">
                                    <a href="{{ route('details-hopital', ['id' => $product->id, 'slug' => Str::slug(Str::limit($product->title, 10))]) }}">{{ Str::limit($product->title, 5) }}
                                    </a>
                                 </h3>
                                 ...
                                 <div class="tp-product-button">
                                    <a class="tp-btn-price w-100 text-center" href="{{ route('details-hopital', ['id' => $product->id, 'slug' => Str::slug(Str::limit($product->title, 10))]) }}">
                                       {{-- <i>
