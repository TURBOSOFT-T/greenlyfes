...
@section('body')
 ...
 <head>
    @section('logement')
        <meta name="description" content="{{ $logement->name ?? ' ' }}">
        ...
        <meta property="og:type" content="logement">
        <meta property="og:price:amount" content="{{ $logement->price }} DT">

        <meta property="og:availability" content="{{ $logement->statut }}">

        <meta property="product:price:amount" content="{{ $logement->price }} DT">

        <meta property="logemment:availability" content="{{ $logement->statut }}">
        <meta name="robots" content="index, follow">


        <meta name="csrf-token" content="{{ csrf_token() }}">
        <title>{{ isset($logement) && $logement->seo_title ? $logement->seo_title :  config('app.name') }}</title>
        <meta name="description" content="{{ isset($logement) && $logement->meta_description ? $logement->meta_description : __(config('app.description')) }}">
        <meta name="author" content="{{ isset($logement) ? $logement->user->name : __(config('app.author')) }}">
        @if(isset($logement) && $logement->meta_keywords)
            <meta name="keywords" content="{{ $logement->meta_keywords }}">
        @endif
            ...
            <div class="tp-product-details-area pt-130">
                                ...
                                <div class="tp-shop-details__tab-content-box mb-20">
                                    <div class="tab-content" id="nav-tabContent">
                                        ...
                                        <div id="main-image" class="tp-shop-details__tab-big-img">
                                            <img id="zoom-image" src="{{ url('public/Image/' . $logement->image) }}"
                                                data-zoom-image="{{ url('public/Image/' . $logement->image) }}"
                                ...
                                <div class="tp-shop-details__tab-btn-box">
                                    ...
                                    <nav>
                                        <div class="nav nav-tab" id="nav-tab" role="tablist">
                                            @if (!empty($logement->images) && is_array(json_decode($logement->images, true)))
                                                @foreach (json_decode($logement->images) as $key => $image)
                                    ...
                                    {{-- <nav>
                                        <div class="nav nav-tab" id="nav-tab" role="tablist">
                                            ...
                                            @foreach (json_decode($logement->images) as $key => $image)
                                                <button class="nav-links  thumbnail-btn" id="nav-{{ $loop->index }}-tab"
                                                    data-bs-toggle="tab" data-bs-target="#nav-{{ $loop->index }}"
                                                    type="button" role="tab" aria-controls="nav-{{ $loop->index }}"
                                                    aria-selected="{{ $loop->first ? 'true' : 'false' }}">
                                                    <img src="{{ url('public/Image/' . $image) }}" alt="">
                                                </button>
                                            @endforeach
                                    ...
                                    <div class="product-additional-tab">
                                            ...
                                            <ul class="nav nav-tabs pro-details-nav-btn justify-content-center"
                                                ...
                                                {{--    <li class="nav-item" role="presentation">
                                                    <button class="nav-links" id="information-tab" data-bs-toggle="tab"
                                                        data-bs-target="#additional-information" type="button"
                                                        role="tab" aria-controls="additional-information"
                                                        aria-selected="false"><span>Contacts</span></button>
                                                </li>
                                                <li class="nav-item" role="presentation">
                                        ...
                                        <div class="tab-content tp-content-tab" id="myTabContent-2">
                                            <div class="tab-para tab-pane fade show active" id="home-1" role="tabpanel"
                                                ...
                                                <p class="mb-20" style="text-align: justify"> {!! $logement->body !!}
                                                </p>
             ...
             <div class="tp-product-area pb-120">
                <div class="container">
                    ...
                    <div class="row">
                        @if ($logement->logements->count() > 0)
                            @foreach ($logement->rooms as $logement)
                                        ...
                                        <div class="tp-product-content-2">
                                            ...
                                            <div class="tp-product-button">
                                                <a class="tp-btn-price w-100 text-center"
                                                    href="{{ url('reservation', ['id' => $logement->id, 'slug' => Str::slug(Str::limit($logement->name, 20))]) }}">
                            ...
                            @endforeach
                        @else
                            <p>Aucune chambre n'est associée à cet appartement.</p>
                        @endif
